...
class SettingController extends Controller
{
    public function updateSetting(SettingRequest $request, $user_id)
    {
        ...
        if (!$user) {
            return apiResponse(404, 'Opps Something Was Wron !');
        }
        $user->update($request->except(['_method', 'image']));

        ImageManger::uploadImages($request, null, $user);

        return apiResponse(200, 'Profile Date Updated Successfully !');
    }
    ...
    public function updatePassword(Request $request, $user_id)
    {
        ...
        if (!Hash::check($request->current_password, $user->password)) {
            return apiResponse(400,'Password does not match');
        }
        $user->update([
            "password" => Hash::make($request->password)
        ]);
        return apiResponse(200,'Your Password changed Successfully');
    }
